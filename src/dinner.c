...
{
	mutex_handle(&philo->fork_first->fork, LOCK);
	write_status(TAKE_FIRST_FORK, philo, DEBUG_MODE);
	mutex_handle(&philo->fork_second->fork, LOCK);
	...
	if (philo->table->nbr_limit_meals > 0
		&& philo->meals_eaten == philo->table->nbr_limit_meals)
		set_bool(&philo->philo_mutex, &philo->is_full, true);
	mutex_handle(&philo->fork_first->fork, UNLOCK);
	mutex_handle(&philo->fork_second->fork, UNLOCK);
}
...
void    dinner_start(t_table *table)
{
    ...
    else
    {
        while (++i < table->philo_nbr)
			thread_handle(&table->philos[i].thread_id, dinner_simulation,
